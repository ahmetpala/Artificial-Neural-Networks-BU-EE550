...
np.random.seed(2020802018) # In Jupyter notebook, seed must be set for each cell random operation
noise = np.random.normal(loc=0.0, scale=0.15, size=(1,15)) # Adding noise with mean zero and sigma=0.15
noise = pd.DataFrame(noise.transpose(), columns = ['noise']) # Converting noise array to Pandas dataframe
...
def rls_algo(x, y, theta, P): 
    inv = np.linalg.inv(np.identity(1) + np.dot(np.dot(x.T, P), x)) # Defining inter variable inv for P
...
final_table.columns = ['Name', 'Model', 'Theta_0', 'Theta_1', 'Theta_2', 'Theta_3', 'RLS Error']
final_table['Name'] = ['Model 1', 'Model 2', 'Model 3', 'Model 4']
final_table['Model'] = ['t_0', 't_0 + t_1*x', 't_0 + t_1*x + t_2*x^2', 't_0 + t_1*x + t_2*x^2 + t_3*x^3']
final_table['Theta_0'].iloc[0] = theta[0][0]

# Note that there is only one parameter and it is not multiplied by any input x.

y_1 = pd.DataFrame(pd.Series(theta[0][0] for x in range(len(dt['x']))), index=np.arange(len(dt['x'])), columns=np.arange(1))
y_1 = y_1[0].astype(float)
print(y_1)

# Least squared result check
np.ones(15).T
np.dot(np.dot((1/np.dot(np.ones(15).T, np.ones(15))), np.ones(15).T), dt['y'])
...
y_2 = dt['x'].apply(lambda x: theta[0][0] + theta[1][0]*x) # Storing estimations from Model 2
...
x = np.array([dt['x'], dt['x'].apply(lambda x: x**2)]).T # Defining x and y as np arrays
y = np.array([dt['y']]).T
...
# Least squared result check
x_lse = np.stack((np.ones(15).T, np.array(dt['x']).T, np.array(dt['x']**2).T))
x_lse = x_lse.T
np.dot(np.dot(np.linalg.inv(np.dot(x_lse.T, x_lse)), x_lse.T), dt['y'])
...
y_3 = dt['x'].apply(lambda x: theta[0][0] + theta[1][0]*x + theta[2][0]*x**2) # Storing estimations from Model 3
...
x = np.array([dt['x'], dt['x'].apply(lambda x: x**2), dt['x'].apply(lambda x: x**3)]).T # Defining x and y as np arrays
y = np.array([dt['y']]).T


n = 4 # Number of parameters = 4 since y(i) = theta_0 + theta_1*x(i) + theta_2*x(i)**2 + theta_3*x(i)**2
theta = np.zeros((n, 1)) # Defining initial value of theta
P = np.identity(n)*100   #Defining initial value of P(t-1)

for k in range(len(x)):
    theta, K, P = rls_algo( np.array([[1, x[k][0], x[k][1], x[k][2]]]).T, y[k], theta, P)
    print(theta)
...
# Least squared result check
x_lse = np.stack((np.ones(15).T, np.array(dt['x']).T, np.array(dt['x']**2).T, np.array(dt['x']**3).T))
x_lse = x_lse.T
np.dot(np.dot(np.linalg.inv(np.dot(x_lse.T, x_lse)), x_lse.T), dt['y'])
...
y_4 = dt['x'].apply(lambda x: theta[0][0] + theta[1][0]*x + theta[2][0]*x**2 + theta[3][0]*x**3 ) # Storing estimations from Model 4
...
RLS_errors = pd.DataFrame(pd.Series(0 for x in range(4)), index=np.arange(4), columns=np.arange(1))
...
for k in range(len(RLS_errors)):
    RLS_errors.iloc[k,1] = ((outputs.iloc[1] - outputs.iloc[k+2])**2).sum()
...
# 6) Presenting final table with estimated parameters and RLS errors for each model
