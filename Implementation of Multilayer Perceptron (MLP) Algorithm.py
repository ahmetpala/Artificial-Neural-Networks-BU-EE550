...
labels_xor =xor(data_xor[:,0],data_xor[:,1]).reshape(len(data_xor),1) # Calculating Labels
...
for epoc in range(epoches):
    ...
    for i in range(len(data_xor)):
        ...
        W1, W2, b1, b2, L = back_propagation(o1, o2, s1, s2, y_hat, y, n, W1, W2, b1, b2)
    Loss = np.append(Loss,((forward_propagation(data_xor.T, W1, b1, W2, b2)[4].reshape(4,1)-labels_xor)**2).sum()) 
...
print("Original predictions",forward_propagation(data_xor.T, W1, b1, W2, b2)[4]) # Original predictions

print("Estimations (Rounded)",np.round(forward_propagation(data_xor.T, W1, b1, W2, b2)[4]).flatten())
...
for epoc in range(epoches):
    ...
    for i in range(len(combined)):
        ...
        o1, o2, o3, s1, s2, s3, y_hat = forward_propagation_2(x, W1, b1, W2, b2, W3, b3)
        W1, W2, W3, b1, b2, b3, L = back_propagation_2(o1, o2, o3, s1, s2, s3, y_hat, y, n, W1, W2, W3, b1, b2, b3)
    Loss_2 = np.append(Loss_2,((forward_propagation_2(x_train.T, W1, b1, W2, b2, W3, b3)[6].reshape(len(x_train),1)-y_train)**2).sum()) 
...
# Plotting the train dataset consisting of 60 data points
predictions = forward_propagation_2(x_train.T, W1, b1, W2, b2, W3, b3)[6] # Predictions for train data
...
predictions = forward_propagation_2(x_test.T, W1, b1, W2, b2, W3, b3)[6] # Predictions for train data
...
for k in range(10):
    ...
    random_indices = np.random.choice(deneme.shape[0], size=120, replace=False) # Selecting randomly 120 rows
...
for epoc in range(epoches):
    ...
    for j in range(len(y_ar)):
        ...
        o1, o2, o3, s1, s2, s3, y_hat = forward_propagation_3(x, W1, b1, W2, b2, W3, b3)
        W1, W2, W3, b1, b2, b3, L = back_propagation_3(o1, o2, o3, s1, s2, s3, y_hat, y, n, W1, W2, W3, b1, b2, b3)
    Loss_3 = np.append(Loss_3,np.mean(-np.multiply(hot_train_targets, np.log(forward_propagation_3(x_ar.T, W1, b1, W2, b2, W3, b3)[6])).sum(axis=0)))
    Accuracy = np.append(Accuracy, accuracy(train_x, train_y, W1, b1, W2, b2, W3, b3))
    print("Epoc =",epoc,"--","Loss =",Loss_3[len(Loss_3)-1],"--","Training Accuracy = %",100*accuracy(train_x, train_y, W1, b1, W2, b2, W3, b3))
    if epoc >= 75: # defining minimum number of epoches
        if (abs(Loss_3[len(Loss_3)-1] - Loss_3[len(Loss_3)-2])/Loss_3[len(Loss_3)-2])*100 <= 1: # Defining minimum threshold
            break
...
for e in range(10):
    print(np.argmax(forward_propagation_3(test_x[20*e,:].reshape(64,1), W1, b1, W2, b2, W3, b3)[6], axis=0))
