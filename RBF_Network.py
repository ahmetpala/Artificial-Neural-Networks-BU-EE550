...
x = np.random.rand(n_of_samples,1) # Producing samples from uniform distribution [0,1]
x = np.sort(x, axis=0)
e = np.random.normal(loc=0.0, scale=0.2, size=n_of_samples).reshape(n_of_samples,1) # Producing errors with zero mean variance 0.2
y = 0.4*np.cos(np.radians(2*180*x)) + 0.6*np.sin(np.radians(7*180*x)) + 0.5 + e # Producing y values


# Visualizing Data Point Pairs
plt.title("Generated Data Points (y = 0.4 cos(2*pi*x) + 0.6 sin(7*pi*x) + 0.5 + e)") 
plt.xlabel("x") 
plt.ylabel("y") 
plt.scatter(x, y)
...
train_indices = random.sample(range(0, n_of_samples-1), train_size) # Producing random index numbers for train set
...
plt.title("Train Data Points (y = 0.4 cos(2*pi*x) + 0.6 sin(7*pi*x) + 0.5 + e)") 
plt.xlabel("Train x") 
plt.ylabel("Train y") 
...
#means = kmeans.cluster_centers_ # This "means" array tried for the RBF network but it gave poor results
                                # Therefore, means are set manually by visual inspection (on the extremum points of train data)
...
def forward(X, W, b, std, means):
    ...
    fi = np.array([fi1, fi2, fi3, fi4, fi5]).reshape(k,1) # fi values concatenation
...
#sigma = (means.max() - means.min())/np.sqrt(2*k) # This sigma value gave poor results.
sigma = 0.08 # Final sigma value is obtained after several trials since it is a hyperparameter for the network
...
print("MSE for test data: ", (0.5*(preds.flatten() - y_test.flatten())**2).mean())
