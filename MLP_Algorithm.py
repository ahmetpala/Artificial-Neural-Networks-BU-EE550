...
# Network Architecture: 2 input neurons, 1 hidden layer
# with 5 hidden neurons, and 1 output neuron
...
for epoc in range(epoches):
    ...
    Loss = np.append(
        ...
        (
            (
                forward_propagation(data_xor.T, W1, b1, W2, b2)[4].reshape(
                    4, 1
                )
                - labels_xor
            )
    ...
    if epoc >= 2500:  # Defining minimum numbers of epoches
        if (
            abs(Loss[len(Loss) - 1] - Loss[len(Loss) - 2])
            / Loss[len(Loss) - 2]
        ) * 100 <= 0.01:
            break
...
print(
    "Original predictions", forward_propagation(data_xor.T, W1, b1, W2, b2)[4]
)  # Original predictions

print(
    "Estimations (Rounded)",
    np.round(forward_propagation(data_xor.T, W1, b1, W2, b2)[4]).flatten(),
)
...
# Network Architecture: 1 input neuron, 4 and 5 hidden neurons
# in 1st and 2nd hidden layers, and 1 output neuron in output layer
# Activation function is tanh in the hidden layers and 2*tanh in output layer
# MSE Loss Function


# Defining forward and back-propagation functions
...
# The Network Architecture with 2000 epoches (maximum)
# and learning rate 0.04 (after some trial)
# Initialization
np.random.seed(2020802018)
# Defining number of neurons in each layer
...
for epoc in range(epoches):
    ...
    Loss_2 = np.append(
        ...
        (
            (
                forward_propagation_2(x_train.T, W1, b1, W2, b2, W3, b3)[
                    6
    ...
    if epoc >= 200:  # defining minimum number of epoches
        if (
            abs(Loss_2[len(Loss_2) - 1] - Loss_2[len(Loss_2) - 2])
            / Loss_2[len(Loss_2) - 2]
        ) * 100 <= 0.5:  # Threshold = 0.1 %
            break
...
# Network Architecture: 64 input neurons, 32 and 24 hidden neurons
# in 1st and 2nd hidden layers, and 10 output neurons in output layer
...
def accuracy(x_ar, y_ar, W1, b1, W2, b2, W3, b3):
    return (
        y_ar.astype("int").flatten()
        == np.argmax(
            forward_propagation_3(x_ar.T, W1, b1, W2, b2, W3, b3)[6], axis=0
        )
    ).sum() / len(y_ar)


# The Network Architecture with 200 epoches (maximum)
# and learning rate 0.001 (after some trial)
    ...
    Loss_3 = np.append(
        ...
        np.mean(
            -np.multiply(
                hot_train_targets,
                np.log(
                    forward_propagation_3(x_ar.T, W1, b1, W2, b2, W3, b3)[6]
                ),
            ).sum(axis=0)
        ),
    )
    Accuracy = np.append(
        Accuracy, accuracy(train_x, train_y, W1, b1, W2, b2, W3, b3)
    )
    print(
        "Epoc =",
        epoc,
        ...
        "--",
        "Training Accuracy = %",
        100 * accuracy(train_x, train_y, W1, b1, W2, b2, W3, b3),
    )
    if epoc >= 75:  # defining minimum number of epoches
        # Defining minimum threshold
        if (
            abs(Loss_3[len(Loss_3) - 1] - Loss_3[len(Loss_3) - 2])
            / Loss_3[len(Loss_3) - 2]
        ) * 100 <= 1:
            break
...
for e in range(10):
    print(
        np.argmax(
            forward_propagation_3(
                test_x[20 * e, :].reshape(64, 1), W1, b1, W2, b2, W3, b3
            )[6],
            axis=0,
        )
    )
